...
// 使用示例     wifi_uart_send_byte("1");
// 备注信息     当模块作为TCP服务器时，发送数据函数默认将数据发送至第一个连接模块的客户端
//-------------------------------------------------------------------------------------------------------------------
uint32 wifi_uart_send_byte (const uint8 data)
{
    ...
    if(WIFI_UART_SERVER_ON == wifi_uart_information.wifi_uart_connect_state)
    {
        if(WIFI_UART_COMMAND == wifi_uart_information.wifi_uart_transfer_mode)
        {
            ...
            uart_write_string(WIFI_UART_INDEX, "1");
            uart_write_string(WIFI_UART_INDEX, "\r\n");

            if(0 == wifi_uart_wait_ack("OK", WAIT_TIME_OUT))                    // 等待模块响应
            {
                wifi_uart_clear_receive_buffer();                               // 清空WiFi接收缓冲区
                uart_write_byte(WIFI_UART_INDEX, data);
                if(0 == wifi_uart_wait_ack("OK", WAIT_TIME_OUT))                // 等待模块响应
                {
                    len = 0;
                }
...
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     WiFi 模块 字符串发送函数
// 参数说明     *str            需要发送的字符串
// 返回参数     uint32          剩余未发送数据长度
// 使用示例     wifi_uart_send_string("123", 3);
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
uint32 wifi_uart_send_string (const char *str)
{
    zf_assert(NULL != str);
    int32 timeout = WAIT_TIME_OUT;
    ...
    if(WIFI_UART_SERVER_ON == wifi_uart_information.wifi_uart_connect_state)
    {
        if(WIFI_UART_COMMAND == wifi_uart_information.wifi_uart_transfer_mode)
        {
            ...
            {
                wifi_uart_clear_receive_buffer();                               // 清空WiFi接收缓冲区
                uart_write_string(WIFI_UART_INDEX, str);
                if(0 == wifi_uart_wait_ack("OK", WAIT_TIME_OUT))                // 等待模块响应
                {
                    len = 0;
                }
        ...
        else
        {
            while(len --)
            {
                ...
                uart_write_byte(WIFI_UART_INDEX, *str);                         // 发送最后的数据
                str ++;
            }
        }
    }
...
#endif
    uart_init(WIFI_UART_INDEX, WIFI_UART_BAUD, WIFI_UART_RX_PIN, WIFI_UART_TX_PIN);     // 初始化WiFi模块所使用的串口

		uart_set_interrupt_config(WIFI_UART_INDEX,UART_INTERRUPT_CONFIG_RX_ENABLE);
    do
    {
